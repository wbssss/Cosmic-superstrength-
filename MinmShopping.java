...
public class MinmShopping {
 private static int MAXCODE = 100;//商品编码的最大值
 private static int SALECOMB = 50;//优惠商品组合数
 private static int KIND = 10; //商品种类
 private static int QUANTITY = 10; //购买某种商品数量的最大值
 ...
 private static int[][] offer = new int[SALECOMB+1][KIND+1];//offer[i][j]: 商品组合的优惠价(j=0)；某种优惠组合中某种商品需要购买的数量
 ...
 public static void main(String[] args){ init(); comp(1); out(); 
}
private static void minicost(){
 int i,j,k,m,n,p,minm; minm = 0;
 for(i=1; i<=b; i++)
...
for(p=1; p<=s; p++){
 ...
 int i,j,n,p,t,code;
 for(i=0; i<51; i++) for(j=0; j<11; j++) offer[i][j] = 0;
 for(i=0; i<11; i++){
 purch[i] = new Commodity(); purch[i].piece = 0;
 purch[i].price = 0; product[i] = 0;
 }
 b = input.nextInt();
 for(i=1; i<=b; i++){
 code = input.nextInt();
